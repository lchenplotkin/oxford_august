...
os.makedirs('scan_gower_adjective_analysis_output', exist_ok=True)
orderfile = open('scan_gower_adjective_analysis_output/adj_order.csv', 'w', newline='', encoding='utf-8')
...
def analyze_adjectives(df, results, current_file):
	...
	for idx, row in df.iterrows():
		...
		scansion = row.get('SCANSION', '')

		# Skip lines with no scansion at all, same as the words/tagging guard below.
	...
	for root, dirs, files in os.walk(csv_dir):
		for file in files:
			...
			try:
				...
				required_columns = ['TAGGING', 'TEXT', 'LINE_NUMBER', 'FILENAME', 'SCANSION']
...
if __name__ == "__main__":
	...
	# Write results
	output_dir = 'scan_gower_adjective_analysis_output'
