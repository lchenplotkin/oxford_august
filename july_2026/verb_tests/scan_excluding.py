...
def analyze_verbs(df, results, text_type, doc, current_filename):
    ...
    for idx, row in df.iterrows():
        ...
        for j in range(len(tags)):
            ...
            violated = False
            reason = ""
            if tag == 'v%inf' and not (word.endswith(('a','i','o','u')) or word.endswith('en') or (word.endswith('e') and not is_silent_e) or word.endswith('vowel')) and word not in ['han','seyn','doon','don','gon','goon','forgon','forgoon','fordoon','fordon']:
                violated = True
                reason += "RULE 1: The infinitive ends in -en or -e unless the stem ends in a vowel."
            if tag.startswith('v%imp') and word.endswith('e') and not is_silent_e and verb_class in ["weak","strong"]:
                violated = True
            ...
            if verb_class == "weak" and tag in ['v%pt_1', 'v%pt_3'] and not ((word.endswith('e') and not is_silent_e) or word.endswith('d') or word.endswith('t')):
                violated = True
...
if __name__ == "__main__":
    ...
    oxford_output_dir = 'scan_excluding_oxford_verb_analysis_output'
    riverside_output_dir = 'scan_excluding_riverside_verb_analysis_output'
    combined_output_dir = 'scan_excluding_combined_verb_analysis_output'
