...
def analyze_verbs(df, results, text_type, doc, current_filename):
	...
	for idx, row in df.iterrows():
		...
		# Skip lines with no scansion for this edition, same guard as elsewhere:
		# we can't tell a silent final -e from a pronounced one without it.
		if pd.isna(scansion) or scansion == '':
			continue

		scans = scansion.split(' ')
		words, headwords, tags = parse_tagged_text(text, tagging, text_type)

		for j in range(len(tags)):
			if j >= len(words) or j >= len(headwords) or j >= len(scans):
				continue
			...
			next_tag = clean_tag(tags[j + 1]) if j + 1 < len(tags) else 'END'
			...
			try:
				is_silent_e = word[-1] == 'e' and scan[-1] == 'x'
			except IndexError:
				is_silent_e = False
			...
			# Rule checks. A final -e that the scansion marks as silent is treated as
			# if it weren't there at all: it can't satisfy a "should end in -e" rule,
			# and it can't trigger a "should NOT end in -e" violation either.
			violated = False
			reason = ""
			if tag == 'v%inf' and not (word.endswith(('a','i','o','u')) or word.endswith('en') or (word.endswith('e') and not is_silent_e) or word.endswith('vowel')):
				violated = True
				reason += "RULE 1: The infinitive ends in -en or -e unless the stem ends in a vowel."
			if tag.startswith('v%imp') and word.endswith('e') and not is_silent_e:
				violated = True
				reason += "RULE 3: imperative (singular) are always endingless"
			if tag == 'v%pt_pl' and not (word.endswith('en') or (word.endswith('e') and not is_silent_e)):
				violated = True
				reason += "RULE 4: Preterite plural verbs end in -e/-en"
			if tag == 'v%pr_1' and not (word.endswith('en') or (word.endswith('e') and not is_silent_e)):
				violated = True
				reason += "RULE 5: First person singular present tense verbs end in -e/-en"
			if tag == 'v%pr_pl' and not (word.endswith('en') or (word.endswith('e') and not is_silent_e)):
				violated = True
				reason += "RULE 6: Present plural verbs end in -e/-en"
			if verb_class == "weak" and tag in ['v%pt_1', 'v%pt_3'] and not ((word.endswith('de') and not is_silent_e) or (word.endswith('te') and not is_silent_e) or word.endswith('d') or word.endswith('t')):
				violated = True
				reason += "Rule 7: Singular weak preterite verbs in 1st/3rd person end in -t(e)/-d(e)"
			if verb_class == "strong" and tag == 'v%ppl' and not ((word.endswith('e') and not is_silent_e) or word.endswith('en')):
				violated = True
				reason += "Rule 8: Past participles of strong verbs end in -e/-en"
			if verb_class in ["strong", "weak"] and tag.startswith("v%prp") and not ((word.endswith('e') and not is_silent_e) or word.endswith('en')):
				violated = True
				reason += "Rule 9: Present participle of strong and weak verbs end in -e"
			if verb_class == "strong" and tag == "v%pt_2" and not (word.endswith('e') and not is_silent_e):
				violated = True
				reason += "Rule 10: 2nd person singular preterit of strong verbs end in -e"
			if verb_class == "strong" and tag in ["v%pt_1", "v%pt_3"] and word.endswith('e') and not is_silent_e:
				violated = True
				reason += "Rule 13: 1st and 3rd person singular strong preterites do not end in -e (Barney)"
			if verb_class == "weak" and tag=="v%pt_2" and word.endswith('e') and not is_silent_e:
				violated = True
				reason += "Rule 14: 2nd person singular weak preterite do not end in -e (Barney)"
			if verb_class == "weak" and tag=="v%ppl" and word.endswith('e') and not is_silent_e:
				violated = True
			...
			if violated:
				...
				# Track successful cases (non-exceptions)
	...
	for root, dirs, files in os.walk(csv_dir):
		for file in files:
			...
			try:
				...
				required_columns = [f'{text_type}_TAGGING', f'{text_type}_TEXT', 'LINE_NUMBER', f'{text_type}_FILENAME', f'{text_type}_SCANSION']
...
def write_results(results, output_dir, text_type):
	...
	# --- Successes CSV (non-exceptions) ---
	with open(os.path.join(output_dir, f'{text_type.lower()}_successes.csv'), 'w', newline='', encoding='utf-8') as csvfile:
		fieldnames = ['headword', 'word', 'class', 'tag', 'pct_of_headword_tag', 'success_count']
		writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
		writer.writeheader()

		# Collect all success entries
		success_entries = []
		...
		# Sort and write
		for entry in sorted(success_entries, key=lambda x: (x['headword'], x['tag'], x['word'])):
...
def write_combined_results(oxford_results, riverside_results, output_dir):
	...
	# Combined successes
	oxford_success_keys = set()
	for headword, tag_data in oxford_results['successes'].items():
		for tag, word_counts in tag_data.items():
			for word in word_counts.keys():
				oxford_success_keys.add((headword, word, tag))

	riverside_success_keys = set()
	for headword, tag_data in riverside_results['successes'].items():
		for tag, word_counts in tag_data.items():
			for word in word_counts.keys():
				riverside_success_keys.add((headword, word, tag))

	all_success_keys = oxford_success_keys | riverside_success_keys

	with open(os.path.join(output_dir, 'combined_successes.csv'), 'w', newline='', encoding='utf-8') as csvfile:
		fieldnames = ['TYPE', 'headword', 'word', 'class', 'tag',
					  'oxford_count', 'riverside_count',
					  'oxford_pct_of_headword_tag', 'riverside_pct_of_headword_tag']
		writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
		writer.writeheader()

		for key in sorted(all_success_keys):
			headword, word, tag = key
			in_oxford = key in oxford_success_keys
			in_riverside = key in riverside_success_keys
			...
			oxford_pct = ''
			if in_oxford:
				oxford_count = oxford_results['successes'][headword][tag][word]
				total_tokens = sum(oxford_results['word_tag_counts'][headword][tag].values())
				word_tokens = oxford_results['word_tag_counts'][headword][tag][word]
				if total_tokens > 0:
					oxford_pct = f"{(word_tokens / total_tokens) * 100:.2f}%"
			...
			riverside_pct = ''
			if in_riverside:
				riverside_count = riverside_results['successes'][headword][tag][word]
				total_tokens = sum(riverside_results['word_tag_counts'][headword][tag].values())
				word_tokens = riverside_results['word_tag_counts'][headword][tag][word]
				if total_tokens > 0:
					riverside_pct = f"{(word_tokens / total_tokens) * 100:.2f}%"
			...
			writer.writerow({
				...
				'class': verb_class,
				'tag': tag,
				'oxford_count': oxford_count,
				'riverside_count': riverside_count,
				'oxford_pct_of_headword_tag': oxford_pct,
				'riverside_pct_of_headword_tag': riverside_pct
			})

	print(f"\nCombined exceptions CSV written: {len(all_keys)} unique exceptions")
	print(f"Combined successes CSV written: {len(all_success_keys)} unique successes")
...
if __name__ == "__main__":
	...
	oxford_output_dir = 'scan_advanced_oxford_verb_analysis_output'
	riverside_output_dir = 'scan_advanced_riverside_verb_analysis_output'
	combined_output_dir = 'scan_advanced_combined_verb_analysis_output'
