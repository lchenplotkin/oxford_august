...
def try_scansion_combination(words: List[str], line_counts: List[List[List[int]]],
	...
	for line_combo in product(*word_options):
		...
		if is_pattern_alternating(pattern_str):
			...
			elif total == target + 1 and feminine_fallback is None:
				# VARIANT E: same relaxed check as A, but exact-target matches always
				# win; remember the first valid feminine match only as a fallback.
