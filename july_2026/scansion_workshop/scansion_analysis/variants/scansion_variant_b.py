...
PREFER_PRONOUNCED_FINAL_E = {"hadde", "wolde", "sholde", "coude", "moste"}
	...
	for i, (start, vowels, end) in enumerate(clusters):
		...
		if len(vowels) == 1:
			if word in ELISION_EXCEPTIONS:
				...
				# VARIANT B: prefer pronounced over silent for common modal preterites
				if word in PREFER_PRONOUNCED_FINAL_E:
					counts = [1, 0]
				else:
					counts = [0, 1]
