...
# Two-threshold abstention (see module docstring for the sweep): blank when
# the chosen reading's corpus confidence (score_pattern, 0-100) is below
# ABSTAIN_MIN_CONFIDENCE regardless of anything else, and hold the line to
# the higher ABSTAIN_DISAGREE_MIN bar when the greedy and corpus-ranked
# picks disagree. Tuned for coverage >= 95% on the seed-42 held-out split
# (95.1% answered @ 86.4% precision; 64% of blanked lines truly wrong).
...
ABSTAIN_MIN_CONFIDENCE = 20
ABSTAIN_DISAGREE_MIN = 40
...
def scan(line: str, target_syllables: int = 10, reference: Dict[str, Dict[str, int]] = None) -> Dict[str, any]:
	...
	if confidence < ABSTAIN_MIN_CONFIDENCE or (disagree and confidence < ABSTAIN_DISAGREE_MIN):
		return [''] * len(words), 0
