...
def try_scansion_combination(words: List[str], line_counts: List[List[List[int]]],
	...
	attempt = 0
	multiplier = 100/(len(list(product(*word_options))))
	order_weight = .5
	for line_combo in product(*word_options):
		...
		if total == target:
			#print(attempt*multiplier, score_pattern(pattern_str,words,reference))
			score = order_weight * (100 - attempt*multiplier) + (1-order_weight) * score_pattern(pattern_str, words, reference)
