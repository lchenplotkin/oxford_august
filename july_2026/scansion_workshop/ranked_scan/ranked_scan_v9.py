"""
ranked_scan_v9.py - ranked_scan_v8.py with preset-dial abstention aimed at
high precision on what it does answer. Selection and word lists are
unchanged from v7/v8; only the veto changed:

- Two new per-line veto signals beyond v6's confidence + disagreement:
  the weakest scoreable word's corpus support (a line is only as
  trustworthy as its shakiest word), and the fraction of words with no
  usable green data (they score 0 into the mean confidence, but a line
  full of them can still land deceptively mid-range).
- ABSTAIN_PRESETS / ABSTAIN_PRESET pick the operating point; held-out
  frontier (seed=42, 50/50 split; deployed runs better since the full
  reference table has seen more words):
    balanced (default)  ~80.5% answered @ ~89.7% precision
    mid                 ~53.5% answered @ ~93.7% precision
    strict              ~37.5% answered @ ~95.3% precision
  The strict preset's rule was re-checked on a seed-7 split (94.6%
  precision), so it isn't threshold overfit. 95%+ precision at high
  coverage is NOT reachable with the current green corpus -- the picker's
  base accuracy is ~84% and no available signal separates its errors
  sharply enough; the frontier lifts as more lines get flagged green.

v8's docstring follows (its 95%-coverage rule is the shape "balanced"
tightens; the constants it documents were replaced by the presets above).

ranked_scan_v8.py - ranked_scan_v7.py with the abstention rule retuned for
a coverage-constrained objective: answer AT LEAST 95% of lines, and within
that budget blank as many wrong readings (and as few right ones) as
possible. Selection and word lists are unchanged from v7.

v7's rule ("blank on ANY greedy-vs-corpus disagreement, OR confidence <
30") can never reach 95% coverage: the unconditional disagreement veto
alone blanks ~4% of lines, which plus ~1.7% hard search failures already
exceeds the 5% blanking budget. Swept alternatives on the held-out split
(seed=42, 50/50; rule shapes: disagree-OR-conf<t, disagree-AND-conf<t,
conf<t only, and the two-threshold hybrid), picking max precision-on-
answered subject to coverage >= 95%:

  v7 rule (for reference):   91.4% answered @ 87.8% precision
  conf<28 only:              95.4% answered @ 86.2%, blanks 28 wrong/17 right
  HYBRID (this version):     95.1% answered @ 86.4%, blanks 32 wrong/18 right

The hybrid: blank if confidence < ABSTAIN_MIN_CONFIDENCE (20), or if the
greedy and corpus picks disagree AND confidence < ABSTAIN_DISAGREE_MIN
(40). I.e. disagreement no longer blanks unconditionally -- it just holds
the line to a higher confidence bar. 64% of what it blanks is genuinely
wrong, vs a ~14% base error rate on answered lines, so blanked lines
remain a strongly enriched manual-review worklist. (Thresholds were tuned
on the same held-out split reported above -- treat those numbers as
slightly optimistic, same caveat as every threshold constant in v6/v7.)

v7's docstring follows.
...
# Preset abstention profiles (see module docstring for the frontier sweep).
# Each is a point on the held-out precision/coverage frontier; deployed
# numbers run better than these (full reference table vs the train-half
# table the sweep used). Signals:
#   conf_min           blank if mean corpus confidence < this
#   disagree_conf_min  ...or if greedy and corpus picks disagree AND
#                      confidence < this (equal to conf_min = disagreement
#                      adds nothing; 101 = disagreement always blanks)
#   word_support_min   ...or if the weakest scoreable word's corpus support
#                      for its pattern here is < this
#   unattested_max     ...or if more than this fraction of the line's words
#                      have no green data (or an unattested pattern) --
#                      such words score 0 into the mean but a line full of
#                      them can still land deceptively mid-range
ABSTAIN_PRESETS = {
	# ~80.5% answered @ ~89.7% precision held out. The default.
	"balanced": {"conf_min": 44, "disagree_conf_min": 44,
				 "word_support_min": 0, "unattested_max": 1.01},
	# ~53.5% answered @ ~93.7% precision held out.
	"mid":      {"conf_min": 56, "disagree_conf_min": 56,
				 "word_support_min": 0, "unattested_max": 1.01},
	# ~37.5% answered @ ~95.3% precision held out (94.6% re-checked on a
	# seed-7 split, so not threshold overfit).
	"strict":   {"conf_min": 60, "disagree_conf_min": 80,
				 "word_support_min": 20, "unattested_max": 0.25},
}
ABSTAIN_PRESET = "balanced"
...
def scan(line: str, target_syllables: int = 10, reference: Dict[str, Dict[str, int]] = None) -> Dict[str, any]:
	...
	disagree = _norm(chosen_pattern) != _norm(corpus_pattern)

	# Per-word veto signals: the weakest scoreable word's corpus support for
	# its pattern in this reading, and the fraction of words with no usable
	# green data at all (absent from the table, or an unattested pattern --
	# both score 0 into the mean confidence, but a line full of them can
	# still land deceptively mid-range).
	word_supports = []
	n_scoreable = 0
	n_unattested = 0
	for word, tok in zip(words, chosen_pattern.split(' ')):
		w1 = minimal_clean(word).lower()
		stripped = re.sub(r'[xX]', '', tok)
		if not stripped:
			continue  # fully elided here -- nothing to vouch for
		n_scoreable += 1
		word_patterns = reference.get(w1)
		pattern_count = word_patterns.get(stripped, 0) if word_patterns else 0
		if pattern_count == 0:
			n_unattested += 1
		else:
			word_supports.append(100.0 * pattern_count / sum(word_patterns.values()))
	min_word_support = min(word_supports) if word_supports else 0.0
	frac_unattested = n_unattested / n_scoreable if n_scoreable else 1.0

	preset = ABSTAIN_PRESETS[ABSTAIN_PRESET]
	if (confidence < preset["conf_min"]
			or (disagree and confidence < preset["disagree_conf_min"])
			or min_word_support < preset["word_support_min"]
			or frac_unattested > preset["unattested_max"]):
		return [''] * len(words), 0
