"""
ranked_scan_v2.py - ranked_scan_v1.py plus three data-driven fixes, each
found by categorizing ranked_scan_v1's disagreements with green-flagged
lines in version_comparison.csv (grouped by green-syllables minus
v1-syllables) and checked against green_word_patterns.json before being
made. As of this version: 1728/2012 green lines match (85.88%), vs v1's
1718/2012 (85.39%) -- net +10 (16 fixed, 6 regressed; see below for one
tweak's specific trade-off).

KEPT:
1. Word-final "-en" (weak verb infinitive/plural endings: liven, driven,
   receiven, open, holden, ...) was getting a [1, 0] option -- optionally
   silent -- from the same general "e before d/r/s/n/th/st can elide"
   branch that (correctly) makes past-tense -ed and plural -es
   syncopatable. Checked every word ending in "-en" with >=3 green
   occurrences in green_word_patterns.json: essentially all of them
   (open, comen, drawen, helpen, holden, liven, maken, receiven, riden,
   serven, slepen, speken, tellen, weren, wommen, wondren, ...) are 100%
   "Su" (2 syllables, both sounded) with zero silent-final-en
   occurrences -- the exceptions (heven, ever, never, other, eyen, ...)
   already have their own VARIABLE_SYLLABLE_WORDS entries and never reach
   this general branch. Split the branch so final "-en" always counts
   [1] (mandatory, no silent option) while -ed/-es/-th/-st keep their
   existing [1, 0] flexibility. Alone: +4 net (9 fixed, 5 regressed) on
   the isolated test -- the regressions are mostly other "-en"-shaped
   words (foryeven, foryeten, hevene) where this line's meter needed the
   silent reading anyway, so this is a real trade-off, not a free win,
   but the data clearly favors "mandatory" as the default.

2. "Troye" (the place name Troy) has no vowel-cluster rule giving it 1
   syllable -- its "oye" cluster falls under the general TWO_SYLLABLES
   rule (tuned for words like "joye", which genuinely varies between 1
   and 2 syllables in the green data). But green_word_patterns.json shows
   "troye" is 1 syllable 4/4 times with zero exceptions. Added a
   VARIABLE_SYLLABLE_WORDS entry forcing it to 1, same pattern as
   Troilus/Caliope/Proigne. Alone: +4 net (4 fixed, 0 regressed).

3. "Deye" (to die) had no 2-syllable option at all (falls through every
   vowel-cluster rule to the default of 1), but green_word_patterns.json
   shows it split 2/3 "Su" (2 syllables) vs 1/3 "S" (1 syllable) -- a
   real single-word exception, not a general rule (a general "diphthong-
   final-unfollowed words get 2 syllables" rule was tried and dropped in
   ranked_scan_v1's ancestor, scansion_v2.py, because it also wrongly
   applied to "ye", the far more common pronoun). Added a
   VARIABLE_SYLLABLE_WORDS entry with [2, 1]. Alone: +2 net (2 fixed, 0
   regressed).

All three were tested in isolation against ranked_scan_v1 on the full
green-flagged set before being combined here; combining them didn't change
any individual fix's direction (still +10 net together, matching the sum
of the isolated nets exactly: 4+4+2=10).

Ranking mechanism (score_pattern, line_confidence, etc.) is unchanged from
ranked_scan_v1.py -- see that file's docstring for how the ranking itself
works. This file only changes syllable-counting rules.
"""
...
def score_seq(w1, w2, t1, t2, BONUS = 10):
	score_add = 0
	if w1.endswith('e') and (not w1.endswith(('ie','oe','ae','ye','ue'))):
		if w1 not in ELISION_EXCEPTIONS and (w2.startswith(('a','e','i','o','u')) or w2 in ELISION_FOLLOWERS):
			if t1.endswith('x') and t2.startswith('u'):
				score_add+=BONUS
		elif t1.endswith('x'):
			score_add-=BONUS
		elif t1.endswith('u'):
		...
		if t1.endswith('S'):
			score_add-=BONUS
	if w1.endswith(('eth','en','er')) and t1.endswith('S'):
		score_add-=BONUS
	#if w1
	return score_add
...
def score_pattern(pattern_str: str, words: List[str], reference: Dict[str, Dict[str, int]]) -> float:
	...
	for word, tok in zip(words, tokens):
		if i<(len(words)-2):
			next_word = words[i+1]
			w2 = minimal_clean(next_word).lower()
			tok2 = tokens[i+1]
		else:
			w2 = 'END'
			tok2 = 'END'
		...
		if w2!='END':
			score += score_seq(w1, w2, tok, tok2)
		i+=1
