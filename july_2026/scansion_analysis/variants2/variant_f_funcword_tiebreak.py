"""
scansion_v2.py - scansion.py plus two validated fixes, chosen empirically by
...
for the harness and scansion_analysis/compare_green_flags.py for the original
gap analysis. Baseline scansion.py: 1244/1534 match (81.10%). This file:
1309/1534 (85.33%), net +65 lines fixed vs regressed.
...
TRIED AND DROPPED (both made things worse -- kept here as a record so they
aren't tried again without re-checking):
...
# Closed-class function words (pronouns, prepositions, conjunctions, articles,
# grammatical adjectives/determiners) mined from OXFORD_TAGGING across the
# whole scansion_tool corpus: any word form whose POS tag is one of these
# categories >=80% of the time it occurs. Used only as a tie-break signal
# (see try_scansion_combination) -- these words are essentially never
# metrically stressed in practice, so among several otherwise-valid
# alternating readings of a line, the one that stresses fewer of them is
# preferred.
FUNCTION_WORDS = {'a', 'about', 'abouten', 'aboven', 'aforyein', 'aftir', 'againes', 'agains', 'ageins', 'agen', 'albeit', 'alle', 'altheigh', 'althogh', 'although', 'amid', 'amidde', 'among', 'amonge', 'amonges', 'an', 'and', 'another', 'anothers', 'any', 'as', 'ascaunce', 'at', 'atte', 'atwixe', 'atwixen', 'ayeines', 'ayeins', 'ayens', 'because', 'befor', 'behind', 'beside', 'besidin', 'betwix', 'betwixe', 'betwixen', 'beyonde', 'bi', 'bifor', 'bihind', 'bihinden', 'bineth', 'bisides', 'bitwene', 'bitwix', 'bitwixe', 'bitwixen', 'biyond', 'bot', 'both', 'but', 'by', 'during', 'ech', 'echon', 'echone', 'echoon', 'eech', 'either', 'emforth', 'endlong', 'er', 'euery', 'evenforth', 'everich', 'everiche', 'everichon', 'everichone', 'everichoon', 'every', 'everychone', 'everychoon', 'few', 'fewe', 'for', 'forthi', 'forthy', 'forwhi', 'forwhy', 'fro', 'from', 'gif', 'he', 'hem', 'hemself', 'hemselve', 'hemselven', 'her', 'herself', 'herselve', 'herselven', 'him', 'himself', 'himselfe', 'himselve', 'himselven', 'hir', 'hires', 'hirs', 'hirself', 'hirselve', 'hirselven', 'his', 'hise', 'hit', 'how', 'howe', 'i', 'ich', 'if', 'ik', 'ilke', 'in', 'inmid', 'into', 'inwith', 'it', 'liche', 'lik', 'm', 'many', 'maugre', 'maugree', 'me', 'mi', 'min', 'mine', 'mony', 'my', 'myn', 'myne', 'myself', 'myselfe', 'myselve', 'myselven', 'n', 'na', 'nan', 'neen', 'negh', 'neither', 'ni', 'no', 'non', 'none', 'noon', 'nor', 'nother', 'nothinge', 'nouther', 'of', 'on', 'oon', 'or', 'other', 'othere', 'otheres', 'others', 'othres', 'our', 'oure', 'ourselven', 'outher', 'outtaken', 'over', 'ovir', 'par', 'same', 'sans', 'saue', 'sauns', 'saving', 'savinge', 'sh', 'she', 'shee', 'sin', 'sith', 'soche', 'som', 'some', 'somme', 'such', 'suche', 'sum', 'swich', 'swiche', 't', 'th', 'thair', 'that', 'thay', 'the', 'thee', 'thei', 'thes', 'these', 'they', 'thi', 'thilke', 'thin', 'thine', 'this', 'thise', 'thiselven', 'thogh', 'thorgh', 'thorugh', 'thorughout', 'thou', 'though', 'thourgh', 'thow', 'throgh', 'through', 'throughout', 'thurgh', 'thurghout', 'thy', 'thyself', 'thyselve', 'thyselven', 'til', 'till', 'to', 'tofore', 'toforn', 'tother', 'touching', 'touchinge', 'toward', 'towardes', 'towards', 'under', 'until', 'unto', 'upon', 'us', 'we', 'wha', 'whan', 'whanne', 'wheither', 'when', 'whenne', 'wher', 'where', 'whether', 'which', 'whiche', 'whil', 'whilk', 'whils', 'who', 'whom', 'whos', 'whoso', 'with', 'within', 'without', 'withoute', 'withouten', 'ye', 'ymel', 'you', 'your', 'youre', 'youres', 'yours', 'yourself', 'yourselve', 'yourselven', 'yow'}
...
def score_pattern(pattern_str: str, words: List[str]) -> int:
	"""
	Penalize readings that put metrical stress on a closed-class function
	word (pronoun, preposition, conjunction, article, grammatical adjective).
	Lower is better. Only monosyllabic-in-this-reading occurrences count,
	since a polysyllabic function word (e.g. "upon") already has its own
	natural stress and isn't the ambiguity this is meant to catch.
	"""
	tokens = pattern_str.split(' ')
	penalty = 0
	for word, tok in zip(words, tokens):
		wl = minimal_clean(word).lower()
		core = re.sub(r'[^Su]', '', tok)
		if core == 'S' and wl in FUNCTION_WORDS:
			penalty += 1
	return penalty
...
def try_scansion_combination(words: List[str], line_counts: List[List[List[int]]],
	...
	Among all valid alternating readings, prefer exact-target-length ones
	over feminine-ending ones (as before), but within each tier break ties
	by minimizing how many closed-class function words end up stressed,
	instead of just taking whichever combination the search reaches first.
	"""
	word_options = []
	...
	best_exact = None
	best_exact_penalty = None
	best_feminine = None
	best_feminine_penalty = None
	...
	for line_combo in product(*word_options):
		...
		if total == target:
			penalty = score_pattern(pattern_str, words)
			if best_exact_penalty is None or penalty < best_exact_penalty:
			...
			if clean and clean[-1] == 'u':
				penalty = score_pattern(pattern_str, words)
				if best_feminine_penalty is None or penalty < best_feminine_penalty:
					best_feminine_penalty = penalty
					best_feminine = {
						"adjusted_counts": adjusted_counts,
						"stress_pattern": pattern_str,
						"total_syllables": total,
