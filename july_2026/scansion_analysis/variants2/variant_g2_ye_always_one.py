...
def analyze_word(word: str, prev_word: str = None, next_word: str = None) -> List[List[int]]:
	...
	for i, (start, vowels, end) in enumerate(clusters):
		...
		# Special multi-vowel cases
			...
			# Word-initial or not, "y" immediately before "e" is always the
			# glide consonant /j/ in Middle English, never a true vowel
			# diphthong nucleus (unlike "ey"/"ay"/"oy"), so this is always
			# one syllable: yes, yet, yerne, foryeten, hye, frye, etc.
			counts = [1]
