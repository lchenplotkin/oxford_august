#THE WORKING PROGRAM AS OF JANUARY 30th for docx output of verb rules alongside ENDINGS.PY both are active
import docx
import os
import re
import csv
...
def analyze_verbs(df, results, text_type, doc, current_filename):
	global inf_vowel_ending
	global rule_1_violations
	global inf_fail
	global inf_e_ending
	global pt_weak_endings
	global inf_en_ending
	global pt_pl_fail
	global pt_pl_e_ending
	global pt_pl_en_ending 
	global pt_pl_fail_dict
		...
		for j in range(len(tags)):
			...
			if tag == 'v%inf' and headword not in["ben",'gon','don','fordon','forgon',"vouchen_sauf"]:
				...
				#	inf_vowel_ending+=1
					...
					inf_fail+=1
					violated = True
					reason += "The infinitive ends in -en or -e."
					rule_1_violations+=1
			
			if tag == 'v%pt_pl' and not (word.endswith('en') or word.endswith('e')): 
				violated = True
				reason += "Preterite plural verbs end in -e/-en"
			if tag == 'v%pr_1' and verb_class in ['weak','strong'] and not (word.endswith('en') or word.endswith('e')):
				violated = True
				reason += "First person singular present tense weak and strong verbs end in -e/-en"
			if tag == 'v%pr_1'  and not (word.endswith('en') or word.endswith('e')):
				violated = True
				reason += "(LOOSE RULE, NOT RIGHT) First person singular present tense verbs end in -e/-en"
			if tag == 'v%pr_pl' and verb_class in ['weak','strong'] and not (word.endswith('en') or word.endswith('e')):
				violated = True
				reason += "Present plural weak and strong verbs end in -e/-en"
			if tag == 'v%pr_pl'  and verb_class in ['irregular','pretpres'] and not (word.endswith('en') or word.endswith('e')):
				violated = True
			...
			if verb_class == "strong" and tag == 'v%ppl' and not (word.endswith('e') or word.endswith('en')):
				violated = True
				reason += "Past participles of strong verbs end in -e/-en"
			if verb_class == "weak" and tag == 'v%ppl' and (word.endswith('e') or word.endswith('en')):
				violated = True
				reason += "Past participles of weak verbs do not end in -e/-en"
			if verb_class == "strong" and tag == "v%pt_2" and not word.endswith('e'):
				violated = True
				reason += "2nd person singular preterit of strong verbs end in -e"
			...
			if verb_class == "weak" and tag=="v%pt_2" and word.endswith('e'):		
				violated = True
				reason += "2nd person singular weak preterite do not end in -e (Barney)"
			if verb_class == "weak" and tag=="v%ppl" and word.endswith('e'):
				violated = True
				reason += "The weak past participle does not end in -e (Barney)"
...
if __name__ == "__main__":
	...
	print(inf_e_ending,inf_en_ending,inf_vowel_ending,inf_fail)
	print('pt_pl e:', pt_pl_e_ending, 'pt_pl en: ', pt_pl_en_ending, 'pt_pl fail:', pt_pl_fail)
	for key in pt_pl_fail_dict.keys():
		print(key)
		print(pt_pl_fail_dict[key])
	print(pt_weak_endings)
