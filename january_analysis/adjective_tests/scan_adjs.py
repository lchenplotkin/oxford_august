#Merged Chaucer Adjective Declension Analysis with Per-File Accuracy Breakdown
import docx 
import os
import re
import string
...
# List of headwords to track in weak declension contexts
TRACKED_HEADWORDS = ['right', 'lift', 'west', 'east', 'north', 'south']

orderfile = open('adjective_analysis_output/adj_order.csv', 'w', newline='', encoding='utf-8')
...
def is_weak_form(prev_tag, prev_word, tag, next_tag):
	...
	weak_triggers = ['demonstrative', 'def_art', 'n%gen', 'pron%gen', 'pron%pl_gen', 'interj', 'pron%fem_gen']
...
def analyze_adjectives(df, results, current_file):
	...
	# Initialize tracked_weak_instances if needed
	if 'tracked_weak_instances' not in results:
		results['tracked_weak_instances'] = []
	...
	for idx, row in df.iterrows():
		if row["MATCH"] != "DIFF":
			...
			riv_scansion = row['RIVERSIDE_SCANSION']
			ox_scansion = row['OXFORD_SCANSION']
			
			if riv_scansion!= '' and ox_scansion != '':

				riv_scans = riv_scansion.split(' ')
				ox_scans = ox_scansion.split(' ')
				...
				for j in range(len(tags)):
					if j >= len(words) or j >= len(headwords) or j >= len(ox_scans):
						continue
					...
					# Track adjective ordering (multiple adjectives in a row)
					...
					try:
						...
						print(ox_scansion, ox_scans)
					

					# Track specific headwords in weak declension (excluding elision and line endings)
					if is_weak and headword in TRACKED_HEADWORDS:# and not is_word_elided and not is_final:
						tracked_record = {
							'headword': headword,
							'word': word,
							'has_e': word.endswith('e'),
							'prev_tag': prev_tag,
							'prev_word': prev_word,
						...
						results['tracked_weak_instances'].append(tracked_record)
					...
					if is_weak:
						...
						if word.endswith('e') and not is_silent_e:
						...
						if word.endswith('e') and tag == 'adj' and not is_silent_e:
...
def process_csv_directory(csv_dir):
	...
	results = {
		...
		'tracked_weak_instances': []  # Tracked headwords in weak declension
	}
	...
	for root, dirs, files in os.walk(csv_dir):
		for file in files:
			...
			try:
				...
				required_columns = ['OXFORD_TAGGING', 'OXFORD_TEXT', 'LINE_NUMBER', 'OXFORD_FILENAME', 'RIVERSIDE_SCANSION', 'OXFORD_SCANSION']
...
def write_results_to_csv(results, output_dir):
	...
	# Write tracked weak instances
	with open(os.path.join(output_dir, 'tracked_weak_instances.csv'), 'w', newline='', encoding='utf-8') as csvfile:
		fieldnames = ['headword', 'word', 'has_e', 'prev_tag', 'prev_word', 'line_number', 'filename', 'context']
		writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
		writer.writeheader()
		for record in results['tracked_weak_instances']:
			writer.writerow(record)
...
if __name__ == "__main__":
	print("Processing Chaucer CSVs...")
	print(f"Tracking headwords in weak declension: {', '.join(TRACKED_HEADWORDS)}")
	
	# Process all CSV files
	results = process_csv_directory(base_csv_dir)
	
	# Write results
	output_dir = 'scan_adjective_analysis_output'
	...
	with open(os.path.join(output_dir, 'analysis_log.txt'), 'w', encoding='utf-8') as log_file:
		...
		log_file.write(f"Tracked weak instances found: {len(results['tracked_weak_instances'])}\n\n")
		...
		files = [
			...
			"tracked_weak_instances.csv - Tracked headwords in weak declension (no elision/line endings)",
			...
			"adj_order.csv - Adjective ordering analysis",
	...
	print(f"Check 'tracked_weak_instances.csv' for tracked headwords: {', '.join(TRACKED_HEADWORDS)}")
