...
PENALTY_NON_E_SILENT = 50	  # Marking non-e character as silent
PENALTY_E_BEFORE_VOWEL_UNSTRESSED = 5  # Final e before vowel-initial word marked unstressed instead of silent
PENALTY_E_STRESSED = 0		 # Marking any e as stressed
PENALTY_MIDDLE_E_SILENT = 0	 # Marking e in middle of word as silent
PENALTY_FINAL_E_STRESSED = 10  # Final e marked stressed
PENALTY_FINAL_E_SILENT = 0
PENALTY_HEADLESS_LINE = 5
PENALTY_FEMININE_ENDING = 20
...
def is_vowel_char(char: str, prev_char: str = None, next_char: str = None, 
	...
	if prev_char is None:
		...
		if prev_lower == 'q' and char_lower == 'u':
			return False
		elif char_lower in 'aeou':
			return True
		elif char_lower in 'iy':
			if prev_lower in 'iy':
				return True
			elif prev_lower in 'aeou':
				return False
			else:
				return True
...
for i, syllable in enumerate(word_analysis.syllables):
	...
	pos, char = syllable.vowel_positions[0]
	if char == 'e' and ((pos == len(word_lower)-1 and (next_word[0] in 'aeiou' or next_word in ELISION_FOLLOWERS)) or len(word_analysis.syllables)>1):
	options = ['u','x','S']
	...
	for combo in product(*all_possibilities):
		...
		# Allow feminine ending (11 syllables ending in unstressed)
			...
			if clean and clean[-1] == 'u':
				...
				if not re.search(r'[^aou]en$', last_word):
					line_penalty += PENALTY_FEMININE_ENDING
				valid = True
		elif total == target_syllables - 1 and is_pattern_alternating(pattern):
			clean = re.sub(r'[^Su]', '', pattern)
			if clean and clean[0] == 'S':
				valid = True
				line_penalty += PENALTY_HEADLESS_LINE
