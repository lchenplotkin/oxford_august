...
# ---- paste your entire scansion engine ABOVE this line ----
# (everything you posted: helpers, classes, scan_line, format_scansion, etc.)
# ------------------------------------------------------------
...
# Format: word -> pattern where capitals indicate stressed vowels
FIXED_STRESS_WORDS = {
    "ladies": "lAdies",  # Stressed 'a', unstressed 'i' and 'e'
    # Add more as needed
}

# Vowel clusters with fixed stress patterns
# Format: cluster -> pattern where capitals indicate stressed, lowercase unstressed, [x] silent
FIXED_STRESS_CLUSTERS = {
    "ioun": "iOUN",  # 'o' stressed, others unstressed (treating as multi-syllable)
    # Add more as needed
}
...
PENALTY_NON_E_SILENT = 10  # Marking non-e vowel as silent
PENALTY_FINAL_E_BEFORE_VOWEL_UNSTRESSED = 5  # Final e before vowel-initial word marked unstressed (should be silent)
PENALTY_FINAL_E_STRESSED = 20  # Final e stressed
PENALTY_FINAL_E_SILENT = 5
PENALTY_MIDDLE_E_SILENT = 2  # e in middle of word marked silent
...
# Penalty values for different scansion choices
...
PENALTY_E_STRESSED = 2         # Marking any e as stressed
...
def group_vowels_into_syllables(word: str, vowel_positions: List[Tuple[int, str]]) -> List[List[List[Tuple[int, str]]]]:
    ...
    3. Adjacent vowels without consonants or u-grouping -> flexible (can be 1 to N syllables)
    ...
    for cluster in clusters:
        # Within a cluster, apply u-grouping rules first
        u_groups = []
        ...
        for i in range(1, len(cluster)):
            ...
            # 'u' after vowel -> same group (unless Latin exception)
            if curr_char == 'u' and not is_latin_exception:
                current_group.append((curr_pos, curr_char))
            else:
                u_groups.append(current_group)
                current_group = [(curr_pos, curr_char)]
        
        if current_group:
            u_groups.append(current_group)
        
        # Now we have u-groups. Each u-group is treated as a single unit.
        # Generate all possible syllable divisions from 1 to len(u_groups)
        
        if len(u_groups) == 1:
            # Only one possibility: all vowels in one syllable
            cluster_possibilities.append([[u_groups[0]]])
        else:
            # Generate all partitions from 1 syllable to N syllables
            possibilities = []
            
            # 1 syllable: all u-groups together
            possibilities.append([sum(u_groups, [])])
            
            # 2 to N syllables: various splits
            for num_syllables in range(2, len(u_groups) + 1):
                # Generate all ways to partition u_groups into num_syllables parts
                partitions = generate_partitions(u_groups, num_syllables)
...
class WordAnalysis:
    ...
    def calculate_penalty(self, next_word: str = None) -> float:
        """
        Calculate penalty for this word's stress assignment based on rules.
        """
        penalty = 0.0
        ...
        for i, syllable in enumerate(self.syllables):
            is_final = i == len(self.syllables) - 1
            is_middle = not is_final and i > 0
            
            for pos, char in syllable.vowel_positions:
                # Check if this is the last character in the word
                is_word_final_char = pos == len(word_lower) - 1
                
                if syllable.stress == 'x':  # Silent
                    if char != 'e':
                        # Penalty for marking non-e as silent
                        penalty += PENALTY_NON_E_SILENT
                    elif is_middle:
                        # Penalty for marking e in middle of word as silent
                        penalty += PENALTY_MIDDLE_E_SILENT
                    elif next_clean and not (is_vowel_char(next_clean[0], None, next_clean[1] if len(next_clean)>1 else None) or next_clean in ELISION_FOLLOWERS):
                        penalty += PENALTY_FINAL_E_SILENT
                
                elif syllable.stress == 'S':  # Stressed
                    if char == 'e':
                        # Penalty for marking e as stressed
                        penalty += PENALTY_E_STRESSED
                
                elif syllable.stress == 'u':  # Unstressed
                    if char == 'e' and is_word_final_char and next_word:
                        # Check if next word starts with vowel
                        next_clean = minimal_clean(next_word.lower())
                        if next_clean and (is_vowel_char(next_clean[0], None, 
                                                       next_clean[1] if len(next_clean) > 1 else None) or next_clean in ELISION_FOLLOWERS):
                            # Penalty for marking final e before vowel as unstressed instead of silent
                            penalty += PENALTY_E_BEFORE_VOWEL_UNSTRESSED
        
        return penalty

def apply_fixed_stress_pattern(word: str, word_analyses: List[WordAnalysis]) -> Optional[WordAnalysis]:
    """
    Check if word has a fixed stress pattern and apply it if found.
    Returns WordAnalysis with fixed pattern or None if not a fixed-pattern word.
    """
    word_lower = word.lower()
    
    # Check for fixed word pattern
    if word_lower in FIXED_STRESS_WORDS:
        pattern = FIXED_STRESS_WORDS[word_lower]
        
        # Find matching syllabification
        for analysis in word_analyses:
            # Try to match the pattern to this syllabification
            vowel_positions = []
            for syllable in analysis.syllables:
                vowel_positions.extend(syllable.vowel_positions)
            
            # Extract vowel chars from pattern (ignoring brackets and case)
            pattern_vowels = []
            i = 0
            while i < len(pattern):
                if pattern[i] == '[':
                    ...
                    while j < len(pattern) and pattern[j] != ']':
                        j += 1
                    if j < len(pattern):
                        pattern_vowels.append(pattern[i+1:j].lower())
                    i = j + 1
                elif is_vowel_char(pattern[i].lower(), 
                                  pattern[i-1].lower() if i > 0 else None,
                                  pattern[i+1].lower() if i < len(pattern)-1 else None):
                    pattern_vowels.append(pattern[i].lower())
                    i += 1
                else:
                    i += 1
            
            # Check if vowel positions match
            if len(vowel_positions) == len(pattern_vowels):
                match = all(v[1] == pv for v, pv in zip(vowel_positions, pattern_vowels))
                ...
                if match:
                    ...
                    for syllable in analysis.syllables:
                        ...
                        # Determine stress for this syllable based on pattern
                        syllable_stressed = False
                        syllable_silent = False
                        
                        for pos, char in syllable.vowel_positions:
                            # Find this vowel in the pattern
                            if pattern_idx < len(pattern):
                                # Skip non-vowel chars in pattern
                                while pattern_idx < len(pattern) and not is_vowel_char(
                                    pattern[pattern_idx].lower(),
                                    pattern[pattern_idx-1].lower() if pattern_idx > 0 else None,
                                    pattern[pattern_idx+1].lower() if pattern_idx < len(pattern)-1 else None):
                                    if pattern[pattern_idx] == '[':
                                        syllable_silent = True
                                        pattern_idx += 1
                                ...
                                if pattern_idx < len(pattern):
                                    if pattern[pattern_idx].isupper():
                                        syllable_stressed = True
                                    pattern_idx += 1
                        
                        if syllable_silent:
                            new_syl.stress = 'x'
                        elif syllable_stressed:
                            new_syl.stress = 'S'
                        else:
                            new_syl.stress = 'u'
                    ...
                    return WordAnalysis(analysis.word, new_syllables, penalty=0)
    
    return None

def check_vowel_cluster_pattern(syllable: VowelSyllable, word: str) -> Optional[str]:
    """
    Check if a syllable's vowel cluster matches a fixed pattern.
    Returns the stress pattern if found, None otherwise.
    """
    # Get the vowel cluster string
    cluster = ''.join(char for pos, char in syllable.vowel_positions)
    cluster_lower = cluster.lower()
    
    # Check against fixed patterns
    if cluster_lower in FIXED_STRESS_CLUSTERS:
        return FIXED_STRESS_CLUSTERS[cluster_lower]
...
def calculate_penalty(word_analysis: WordAnalysis, stress_pattern: List[str], 
    ...
    for i, (syllable, stress) in enumerate(zip(word_analysis.syllables, stress_pattern)):
        ...
        for pos, char in syllable.vowel_positions:
            ...
            if stress == 'S' and char == 'e' and is_final_char:
            ...
            if stress == 'u' and char == 'e' and is_final_char and next_word_vowel_initial:
                # Final e before vowel-initial word marked unstressed (should be silent)
                penalty += PENALTY_FINAL_E_BEFORE_VOWEL_UNSTRESSED
...
def line_meter_penalty(pattern: str, syllable_count: int, target: int = 10) -> int:
    ...
    +10: headless line (SuSuSuSuS)
    +10: feminine ending (uSuSuSuSuSu)
    """
